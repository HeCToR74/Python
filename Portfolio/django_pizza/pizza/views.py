...
def build_pizza(request, messages=None):
    ...
    context = {
        ...
        't_regular': Topping.objects.filter(price=1), #FIXED
        # regular toppings are less than $2
        't_premium': Topping.objects.filter(price=2), #FIXED
...
def show_tab(request):
    ...
    grand_total = 0
    for tab in tab_obj:               # FIXED
        toppings_price = sum([tp.price for tp in tab[2]])  # FIXED
        grand_total += tab[0].price + tab[1].price + toppings_price # FIXED
...
def edit_pizza(request):
    ...
    try:
        ...
        index = request.GET['pizza_index'] # FIXED
        index = int(index)
    except ValueError:
        return render(request, "pizza/error.html",
                      {'error_list': ['Invalid pizza choice']})
...
def save_edits(request):
    try:
        ...
        return redirect("show_tab") # FIXED
    except KeyError:
        return render(request, "pizza/error.html",
                                   {'error_list': ['missing information']})
    except Crust.DoesNotExist:
...
def list_invoices(request):
    try:
        customer_id = int(request.GET['customer_id']) #FIXED
